        ...
        fn exhaustive_byte_arithmetic() {
            macro_rules! test_type {
                ($t:ty) => {{
                    ...
                    check!(add, |a: $t, b: $t| (a as i128 + b as i128)
                        as $t);
                    check!(sub, |a: $t, b: $t| (a as i128 - b as i128)
                        as $t);
                    check!(mul, |a: $t, b: $t| (a as i128 * b as i128)
                        as $t);
                    ...
                    macro_rules! check {
                        ($op:ident, $reference:ident) => {{
                            ...
                            for (i, (&x, &actual)) in values.iter().zip(&result).enumerate() {
                                let expected = crate::conformance::oracle::real(
                                    x as f64,
                                    $bits,
                                    stringify!($reference)
                                ) as $t;
            ...
            macro_rules! dtype {
                ($t:ty, $bits:expr) => {{
                    ...
                    macro_rules! check {
                        ($op:ident, $operator:tt) => {{
                            ...
                            for ((a, b), actual) in a.iter().zip(&b).zip(result) {
                                let reference = crate::conformance::oracle::real_binary(
                                    *a as f64,
                                    *b as f64,
                                    $bits,
                                    stringify!($op)
                    ...
                    for size in [1, 63, 64, 129, 8193] {
                        assert_eq!(
                            input(vec![<$t>::NEG_INFINITY; size])
                                .max_all()
                                .unwrap(),
                            <$t>::NEG_INFINITY
                        );
                        assert_eq!(
                            input(vec![<$t>::INFINITY; size]).min_all().unwrap(),
                            <$t>::INFINITY
                        );
                        let mut v = vec![1 as $t; size];
                        v[size - 1] = <$t>::NAN;
            ...
            macro_rules! dtype {
                ($t:ty,$bits:expr) => {{
                    ...
                    for (i, ((&a, &b), v)) in a.iter().zip(&b).zip(actual).enumerate() {
                        let pow = crate::conformance::oracle::real_binary(a as f64, b as f64, $bits, "pow")
                            as $t;
        ...
        fn complex_unary_oracle_and_predicates() {
            macro_rules! dtype {
                ($t:ty) => {{
                    ...
                    let abs = input(vec![Complex::<$t>::new(3., 4.)])
                        .abs()
                        .unwrap();
        ...
        fn complex_binary_oracle_and_branches() {
            macro_rules! dtype {
                ($t:ty) => {{
                    ...
                    macro_rules! check {
                        ($op:ident) => {{
                            let expression = input(left.clone())
                                .$op(input(right.clone()))
                                .unwrap();
